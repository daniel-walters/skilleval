...
3. **Both runners:** Cursor keeps one `Agent` and loops `send` → `wait`; Claude captures `session_id` and uses `options.resume` for follow-ups (not `continue: true`, which races under parallel attempts).
